...
public class TestBase {
    ...
    public void loadPropFile() {
        ...
        try {
           FileInputStream fis = new FileInputStream(filePath_testDataProp);
            testData.load(fis);
        } catch (IOException e) {
...
//        startServer();
        ...
        switch (device) {
            case("TECNO-KJ5"):
                log.info("The test is running on TECNO KJ5");
                DeviceCaps.TECNOKJ5();
                break;
            case("Emulator"):
    ...
    public void validatePasswordRequirements(WebElement el, String message) {
        // Wait for the "OK" button on the registration complete page
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(12)); // Adjust timeout as needed
        try {
            wait.until(ExpectedConditions.visibilityOf(el));
            throw new AssertionError("There is a Bug here the user: " + message );
        } catch (TimeoutException e) {
            System.out.println("Test passed, user unable to complete registration with invalid passwords requirements");
        }
    }
